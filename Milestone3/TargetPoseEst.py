...
# Helpers
# -----------------------------
def is_inside_arena(x: float, y: float, bound: float = ARENA_BOUND) -> bool:
    x = float(x); y = float(y)
...
def expected_ratio_for_class(target_dimensions_dict: Dict[str, Tuple[float,float,float]],
    ...
    """Expected bbox aspect ratio ≈ physical width/height (crude but useful)."""
    w, _, h = target_dimensions_dict[target_class]
    return max(1e-9, w / h)

def bbox_ratio_ok(target_class: str, bbox: List[float] | Tuple[float,float,float,float],
                  target_dimensions_dict: Dict[str, Tuple[float,float,float]],
                  tol: float = 0.15) -> bool:
    """Enforce |(w/h) - expected| / expected ≤ tol."""
...
class FruitRanger:
    ...
    def from_bbox_height(self,
        ...
        This only works with a SINGLE bounding box. Rerun if multiple boxes are present.
    ...
    def from_ground_ray(self,
        ...
        Implement when camera height & pitch are known.
        """
